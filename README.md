...
- 底部工具栏暂时隐藏，默认使用笔模式并在停笔后自动提交；清空、橡皮和手动发送保留在代码中，等待后续重新设计
- 开书采用可见的分段动效：书脊抽出、实体皮革书悬浮、封面向左翻开、纸页连续翻动、羊皮纸铺满屏幕
- 书架加入克制的烛光、漂浮尘埃、符文微光和皮革烫金层，纸面保留旧羊皮暗角与墨迹质感
...
- 外国人物的回信跟随用户提问语言，并参考人物作品在该语言中的原作或通行翻译传统
- 孔子与人皮纸固定使用中文；孔子采用清楚可读的半文半白，外国人物中文回复采用成熟译本语感，不统一套用文言文
- 跨语言由界面作为透明翻译层，人物不假称掌握时代之外的语言，也不照搬特定现代译者
- 内置人物由服务器白名单维护；自定义人物只接受限长结构化资料，不接受客户端任意系统提示词
- 每本书独立的本地历史记录、回复偏好和可编辑长期记忆
- 英文回信优先使用 Dancing Script 艺术手写体，并以 IM FELL English / Uncial Antiqua 回退；中文使用 ZCOOL XiaoWei 与楷体回退
- PWA 主屏幕入口和人物直达路由
- 无 Key 时明确提示无法识别手写内容，不返回冒充真实识别的人物答案
...
- 孔子：以《论语》原文、注疏和可靠全译本为谱系；纸面回信固定使用清楚可读的中文，可采用半文半白或浅近文言
...
未配置模型时，网站会明确提示无法读取手写内容和判断书写语言。接入真实识别与人物回复时，复制环境文件：
