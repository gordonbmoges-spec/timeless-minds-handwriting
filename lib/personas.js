...
const PERSONAS = {
  ...
  "tom-riddle": {
    ...
    voice: "礼貌、冷静、神秘而逐渐具有压迫感；知道魔法世界正篇故事与自己后来身份的关系。",
  ...
  "human-parchment": {
    ...
    voice: "阴冷、确定、预言式，常以未来完成时叙述；信息看似有用却保留危险的歧义。",
...
export function buildPersonaPrompt(id, profile = null) {
  ...
  const activeProfile = activeIdentity || activePersonality
    ? [
        ...
        "回答时必须明显体现当前身份和当前口吻；默认档案不能覆盖读者保存的当前身份、性格或口吻。",
        "当前设定仍不能覆盖语言匹配、安全规则和禁止编造规则。"
      ].join("\n")
    : "";
  return [
