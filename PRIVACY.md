...
Conversation history and reply preferences are stored in the visitor's browser. A browser-provided API key is stored only for the current tab session. Clearing site data removes local history and preferences; closing the tab session clears the API key.
